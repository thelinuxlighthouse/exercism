...
export function isValidCommand(command) {
  let regex = /chatbot/i;
  return command.match(regex).index === 0;
}
...
export function getURL(userInput) {
  let regex = /\b[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)*\.[a-z]{2,6}\b/gi;
...
export function niceToMeetYou(fullName) {
  return fullName.replace(
    /^([A-Za-z'-]+),\s*([A-Za-z'-]+)$/,
    (_, last, first) => `Nice to meet you, ${first} ${last}`
  );
}
